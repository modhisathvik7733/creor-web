        ...
        <div className="flex-1 space-y-6 lg:min-w-0">
          ...
          <div className="rounded-xl border border-border bg-card">
            ...
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                <span className="rounded-full bg-foreground/[0.07] px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                  Current
                </span>
              </div>
              <div className="flex items-center gap-3">
            ...
            <div className="grid grid-cols-2 divide-x divide-border">
              ...
              <div className="px-5 py-4">
                ...
                {quota?.monthly.max !== null && quota?.monthly.max !== undefined ? (
                  ...
                  <p className="mt-1 text-[11px] text-muted-foreground">No limit</p>
                )}
              </div>
          ...
          <div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      ...
                      {isCurrent && (
                        <span className="rounded-full bg-foreground/[0.07] px-1.5 py-px text-[9px] font-medium uppercase tracking-wide text-muted-foreground">
                          Current
                        </span>
                      )}
                      {isPending && (
                        <span className="rounded-full bg-blue-500/10 px-1.5 py-px text-[9px] font-medium tracking-wide text-blue-500">
                          Pending
                        </span>
                      )}
                    </div>
                    ...
                    <ul className="mt-2.5 space-y-1.5">
                      {plan.features.map((f) => (
                        <li key={f} className="flex items-start gap-1.5 text-xs text-muted-foreground">
                          <span className="mt-px text-foreground/50">·</span>
                          {f}
                        </li>
                      ))}
                    </ul>
                    {!isCurrent && !isFree && isPending && (
                      <button
                        ...
                        className="mt-3 flex w-full items-center justify-center gap-1 rounded-lg border border-blue-500/30 py-2 text-xs font-medium text-blue-400 transition-colors hover:bg-blue-500/10 disabled:opacity-50"
                      >
                    ...
                    {!isCurrent && !isFree && !isPending && (
                      <button
                        ...
                        className={`mt-3 flex w-full items-center justify-center gap-1 rounded-lg py-2 text-xs font-medium transition-colors disabled:opacity-50 ${
                          isUpgrade
            ...
            <p className="mt-3 flex items-center gap-1.5 text-[11px] text-muted-foreground">
              ...
              Upgrades are immediate (prorated). Downgrades at cycle end. USD.
            </p>
          </div>
        </div>
        ...
        <div className="w-full space-y-6 lg:w-[42%]">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-1 rounded-lg border border-border px-2.5 py-1.5">
              ...
              <>
                <div className="divide-y divide-border">
                  {visiblePayments.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className={`flex items-center justify-between border-l-2 px-4 py-2.5 ${borderColor}`}
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-muted-foreground">{formatDate(p.timeCreated)}</p>
                        </div>
                ...
                {payments.length > 5 && (
                  <button
                    ...
                    className="w-full border-t border-border py-2.5 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground"
                  >
                    {showAllPayments ? "Show less" : `Show all (${payments.length})`}
                  </button>
                )}
              </>
            )}
          ...
          {/* Test Reset — inline */}
          <div className="flex items-center justify-between rounded-xl border border-red-500/20 bg-card px-4 py-3">
            <span className="text-xs text-muted-foreground">
              <span className="font-medium text-red-400">Test:</span> Reset all billing data
            </span>
            <button
